...
function negotiateContentProducer(req, logWarning, options) {
  ...
  return resolveOperation(contentType, options);
}

function resolveOperation(contentType, options) {
  if (!options.jsonResTemplate) {
    var sendJson = require('./send-json');
  } else {
    var sendJson = require(options.jsonResTemplate);
  }

  if (!options.htmlResTemplate) {
    var sendHtml = require('./send-html');
  } else {
    var sendHtml = require(options.htmlResTemplate);
  }

  if (!options.xmlResTemplate) {
    var sendXml = require('./send-xml');
  } else {
    var sendXml = require(options.xmlResTemplate);
  }
